...
episode count and duration, and channel count. It uses exact 56/12/12 subject
capacities, deterministic greedy assignment, improving pairwise swaps, and
SHA-256 seed tie-breaking. Waveform embeddings, learned features, predictions,
errors, and performance are prohibited inputs.
...
event identities, quality state, and exclusion reason. They are not immediately
reduced to a binary value.
...
EDB is not described as independent external validation. The ten verified
source correspondences, rescaling limitation, and conservative same-subject
exclusion policy are recorded in `docs/CROSS_DATASET_PROVENANCE.md`.
